...
def addNoiseToImage(img, noise, noise_Intensity):
    if noise == 's&p':
        print("Adding S&P Noise: wT = " + str(noise_Intensity['whiteThreshold']) + "; bT = " + str(
            noise_Intensity['blackThreshold']))
        ...
        img[distData < noise_Intensity['whiteThreshold']] = 255
        # set ones above blackThreshold to black
        img[distData > noise_Intensity['blackThreshold']] = 0
        return img
    else:
        if noise == 'gaussian':
            print("Adding Gaussian noise: sigma = " + str(noise_Intensity['sigma']) + '; mean = ' + str(
                noise_Intensity['mean']))
            return noise_Intensity['sigma'] * np.random.randn(img.shape[0], img.shape[1]) + img + noise_Intensity['mean']
        else:
            return img
...
def main(p1, p2):
    ...
    imgOriginal, imgOriginalFft, imgOriginalMags = readImage(p1)
    # image (we are searching in), lets call it transformed
    imgTransformed, imgTransformedFft, imgTransformedMags = readImage(p2)
